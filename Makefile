...
run: install ## Install into PREFIX and run it in debug mode (via the uv env)
	uv run python $(PREFIX)/bin/hidamari_prism -d
...
pot: $(BUILDDIR) ## Regenerate po/hidamari_prism.pot (with a filled-in header)
	meson compile -C $(BUILDDIR) hidamari_prism-pot
	sed -i \
		-e "s/SOME DESCRIPTIVE TITLE\./Hidamari_Prism - Video wallpaper for Linux/" \
		-e "s/Copyright (C) YEAR THE PACKAGE'S COPYRIGHT HOLDER/Copyright (C) 2022 Jeff Shee (jeffshee8969@gmail.com)/" \
		po/hidamari_prism.pot

update-po: $(BUILDDIR) ## Merge the template into the per-language .po files
	meson compile -C $(BUILDDIR) hidamari_prism-update-po
