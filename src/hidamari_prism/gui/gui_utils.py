...
from hidamari_prism.commons import LOGGER_NAME
from hidamari_prism.utils import is_flatpak, is_static_image
...
def get_thumbnail(video_path, list_store, idx):
    ...
    try:
        # Static images can be previewed directly from the file.
        if is_static_image(video_path):
            list_store[idx][0] = GdkPixbuf.Pixbuf.new_from_file_at_size(video_path, -1, 160)
            return
        ...
        if thumbnail:
            list_store[idx][0] = GdkPixbuf.Pixbuf.new_from_file_at_size(thumbnail, -1, 160)
